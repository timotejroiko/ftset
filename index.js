...
/*
	get(n) {
		let size = this._size;
		let data = this._data;
		let delim = this._delimiter;
		if(n === 0) {
			let a1 = data.indexOf(delim);
			return data.slice(0, a1);
		}
		if(n === size - 1) {
			let a1 = data.lastIndexOf(delim);
			return data.slice(a1 + delim.length);
		}
		n = Number(n);
		if(!Number.isInteger(n) || n < 0 || n >= size) { return; }
		if(n > size / 1.3) {
			let i = size;
			let b = data.length;
			while(i > n) {
				b = data.lastIndexOf(delim, b - 1);
				i--;
			}
			let a = data.lastIndexOf(delim, b - 1) + delim.length;
			return data.slice(a,b);
		}
		let i = 0;
		let a = 0;
		while(i < n) {
			a = data.indexOf(delim, a) + delim.length;
			i++;
		}
		let b = data.indexOf(delim, a);
		return data.slice(a,b);
	}
	*/
/*
	slice(start, end = this._size) {
		let size = this._size;
		let data = this._data;
		let delim = this._delimiter;
		if(typeof start !== "number" || typeof end !== "number") { throw new Error("Argument must be a number"); }
		if(start < 0) {
			start += size;
			if(start < 0) { start = 0; }
		} else if(start > size) {
		...
		if(start >= end) { return new this.constructor("", delim); }
		let n1;
		let n2;
		let i;
		let a;
		if(start === 0) {
			n1 = start;
		} else if(start > size / 1.3) {
			i = size;
			a = data.length;
			while(i > start) {
				a = data.lastIndexOf(delim, a - 1);
				i--;
			}
			n1 = data.lastIndexOf(delim, a - 1) + delim.length;
		} else {
			i = 0;
			a = 0;
			while(i < start) {
		...
		if(end === size - 1) {
			n2 = void 0;
		} else if((end - start) > (size - start) / 1.3) {
			i = size;
			a = data.length;
			while(i > end) {
				a = data.lastIndexOf(delim, a - 1);
			...
			n2 = data.lastIndexOf(delim, a - 1) + delim.length;
		} else {
			i = start;
			a -= delim.length;
			while(i < end) {
		...
		let result = data.slice(n1,n2);
		return new this.constructor(result, delim);
	}
	*/
/*
	splice(start, end = this._size) {
		let size = this._size;
		let data = this._data;
		let delim = this._delimiter;
		if(typeof start !== "number" || typeof end !== "number") { throw new Error("Argument must be a number"); }
		...
		let result = data.slice(n1, n2);
		let s1 = n1 ? data.slice(0, n1) : "";
		let s2 = n2 ? data.slice(n2 + delim.length) : "";
		this._data = s1 + s2;
		this._size = size - (end - start);
		return new this.constructor(result, delim);
	}
	*/
