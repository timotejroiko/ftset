...
## Stringbase VS Set VS Array

Just like a Set, Stringbase is a non-indexed datastore, it only stores values and not indexes, but it also implements some features from Arrays which makes it much more flexible than a Set. However, unlike a Set, Stringbase does not automatically prevent duplicate items for performance reasons. If one needs to prevent duplicates, they should always call `.has()` before inserting new data.

Stringbase is something in between a Set and an Array, from an API standpoint Stringbase is 100% compatible with the Set API but with a few additional methods borrowed from Arrays.
...
Stringbase provides fast string matching features by storing its entire dataset as a single contiguous string. This makes the process of finding items by string matching and by regex patterns an order of magnitude faster than looping over arrays, sort of like an in-memory grep.

Stringbase excels at storing a large number of strings with the purpose of quickly searching through them but may not be indicated for heavy read/write usage, check the benchmarks at the end of this readme.
...
*/
// much faster than array.filter() or any other array-like iteration method
```

```js
// using stringbase.matchAll()
time = performance.now();
result = sb.matchAll(/eris/i);
console.log(performance.now() - time);
console.log(result);
/*
=> 3.0929999351501465
...
Getter for the number of items stored in the dataset.

### `.length` -> Number

Getter for the total string length of the dataset.

### `.valueOf()` -> Number

Alias for `.length`. Called automatically when coerced to number (when doing math operations for example).

### `.toString()` -> String

Returns the entire dataset as a string. Called automatically when coerced to string (when concatenated or stringified for example).
...
Returns the first item that partially matches the query. Case sensitive.

* `query` -> A string to search for.

### `.findAll(query)` -> Array

Returns all items that partially match the query. Case sensitive.
...
Appends a number of items to the end of the dataset.

* `data` -> The data to append, can be a `String`, an `Array`, an instance of `Stringbase` or an `Iterable`.

### `.forEach(fn)` -> void

Executes a function on each item of the dataset.

* `fn(item)` -> The function to execute. Provides the current item being iterated.

### `.map(fn)` -> Stringbase

Executes a function on each item of the dataset and returns a new instance of `Stringbase` from the results.
...
Stringbase implements the `Iterable` and `Iterator` protocols so you can loop or iterate over an instance of Stringbase directly.
...
Insert operations are surprisingly on pair with arrays, except for array.unshift which is absolutely terrible with a large number of items. Arrays are not designed for adding items to the beginning because the entire array needs to be reindexed, Stringbase on the other hand has no problems with it. Sets cant add to the beginning and are slower because they check for value uniqueness on every insert.

|| 99999 x 10 char | 9999 x 1000 char |
|-|-|-|
| array.push() | 53.147 | 1571.854 |
| set.add() | 71.045 | 1911.027 |
| stringbase.push() | 54.402 | 1595.230 |
| array.unshift() | 17773.719 | 2077.918 |
| stringbase.unshift() | 52.290 | 1519.934 |

Search operations are what stringbase is designed for, delivering much faster performance accross the board.

|| 99999 x 10 char | 9999 x 1000 char |
|-|-|-|
| array.find(includes) near beginning |  0.081 | 0.061 |
| stringbase.find() near beginning |  0.017 | 0.012 |
| array.find(includes) near end | 6.319 | 159.263 |
| stringbase.find() near end | 0.565 | 2.734 |
| array + Math.random | 0.046 | 0.009 |
| set + Array.from + Math.random | 0.681 | 0.051 |
| stringbase.random() | 0.078 | 0.009 |
| array.filter(includes) | 7.693 | 4.573 |
| stringbase.findAll() | 0.367 | 3.142 |
| array.filter(regex) | 11.327 | 8.152 |
| stringbase.matchAll() | 0.943 | 7.965 |

Delete operations have mixed results, with Sets being the fastest at deleting, Arrays at popping and Stringbase at shifting.

|| 99999 x 10 char | 9999 x 1000 char |
|-|-|-|
| array.splice() | 0.344 | 0.083 |
| set.delete() | 0.051 | 0.006 |
| stringbase.delete() | 0.368 | 2.810 |
| array.pop() | 3.361 | 2.282 |
| stringbase.pop() | 13.276 | 18.775 |
| array.shift() | 5133.597 | 3.788 |
| stringbase.shift() | 8.616 | 3.083 |
...
| for(let i of stringbase) | 9.509 | 3.165 |
| array.forEach() | 3.875 | 1.251 |
| set.forEach() | 3.085 | 1.106 |
| stringbase.forEach() | 4.746 | 3.225 |
| array.map() | 4.540 | 1.533 |
| stringbase.map() | 5.516 | 4.597 |

## Caveat

javascript string concatenation makes use of v8's [ConsString](https://thlorenz.com/v8-dox/build/v8-3.25.30/html/df/d91/objects_8h_source.html#l09140) class which improves performance by not concatenating immediately and instead batch-processing several concats at once when needed. This may cause the first read operation in js to be much slower than normal when performed after a write operation because the read will force any pending concatenations in v8 to be processed before the string becomes readable. This slowness scales with data size, therefore Stringbase may not be recommended for general purpose read/write usage on large datasets, it works much better for read-only or read-many/write-few use cases in those situations.
